...
def main():
    ...
    for i in range(len(results["artists"])):
        if "begin-area" in results["artists"][i] and results["artists"][i]["name"] == "Queen":
    ...
    for i in range(len(results["artists"])):
        if "aliases" in results["artists"][i]:
            all_aliases = results["artists"][i]["aliases"]
            spanish_alias = [a for a in all_aliases if a["locale"]=="es"]
            print (spanish_alias)

    # Nirvana Disambiguation
    results = query_by_name(ARTIST_URL, query_type["simple"], "Nirvana")
    nirvanas = [a for a in results["artists"] if a["name"] == "Nirvana"]
    for i in range(len(nirvanas)):
        if "disambiguation" in nirvanas[i]:
            pretty_print(nirvanas[i]["disambiguation"])
    # when one direction was created
    results = query_by_name(ARTIST_URL, query_type["simple"], "One direction")
    pretty_print(results["artists"][0]["life-span"]["begin"])
    # print ("Access Lady Gaga")
    # for i in range(len(results["artists"])):
    #     if results["artists"][i]["name"] == "Lady Gaga":
    #         pretty_print(results["artists"][i]["begin-area"]["id"])
