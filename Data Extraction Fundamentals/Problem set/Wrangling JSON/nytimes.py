...
def article_overview(kind, period):
    ...
    for d in data:
        k_section = d["section"]
        v_title = d["title"]
        titles.append({k_section:v_title})
    
    for d in data:
        for i in range(len(d["media"])):
            for j in range(len(d["media"][i]["media-metadata"])):
                if d["media"][i]["media-metadata"][j]["format"] == "Standard Thumbnail":
                    urls.append(d["media"][i]["media-metadata"][j]["url"])
